...
def kurHesapla():
    try:
        ...
        if kursec == "₺ TL":
            ...
            kursec_orani = roundeddolar
        elif kursec == "€ EURO":
            kursec_orani = roundedeuro
        else:
            kursec_orani = 0

        sonuc = (miktar * girilen_kur_orani) / kursec_orani

        lbl_hesapsonuc.config(text=f"Sonuç: {miktar} {girilen_kur} = {sonuc:.2f} {kursec}")
    except ValueError:
